...
import 'get_market_data_usecase_test.mocks.dart';
...
void main() {
  ...
  setUp(() {
    ...
    useCase = GetMarketDataUseCase(repository: mockRepository);
  });
  ...
  test('should get market data from repository', () async {
    // arrange
    when(
      mockRepository.getMarketData(tSymbol),
    ...
    verify(mockRepository.getMarketData(tSymbol));
  ...
  test(
    ...
    () async {
      ...
      when(
        mockRepository.getMarketData(tSymbol),
      ).thenAnswer((_) async => Left(AssetFailure.marketDataNotFound('Error')));
